...
import { removeFromCart, updateQty, selectCartTotal } from '../redux/slices/cartSlice';
...
export default function CartPage() {
  ...
  if (items.length === 0) return (
    <div className="max-w-2xl mx-auto px-4 py-24 text-center page-enter">
      ...
      <h2 className="font-display text-2xl text-gray-600 mb-2">Your cart is empty</h2>
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 page-enter">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {items.map(item => (
            <div key={item._id} className="card p-4 flex gap-4 border border-gray-100">
              {/* Image */}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start gap-2">
                  <div>
                    <p className="text-xs text-primary font-semibold uppercase tracking-wide">{item.category}</p>
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <div className="flex items-center border border-gray-200 rounded-full overflow-hidden text-sm">
                    ...
                    >−</button>
                    <span className="px-3 py-1.5 font-semibold min-w-[2rem] text-center">{item.qty}</span>
                    <button
                      onClick={() => dispatch(updateQty({ id: item._id, qty: Math.min(item.stock, item.qty + 1) }))}
                      className="px-3 py-1.5 hover:bg-gray-100 transition-colors font-bold"
                    >+</button>
                  </div>
                  {/* Price */}
        ...
        <div className="lg:col-span-1">
          <div className="card p-6 border border-gray-100 sticky top-24">
            <h2 className="font-display text-xl font-bold text-gray-800 mb-5">Order Summary</h2>
            <div className="space-y-3 text-sm text-gray-700">
              ...
              <div className="border-t border-gray-200 pt-3 flex justify-between text-base font-bold">
                <span>Total</span>
                <span className="text-primary">₹{grand.toLocaleString('en-IN')}</span>
              </div>
            </div>
            ...
            <Link to="/products" className="block text-center text-sm text-gray-500 hover:text-primary mt-3 transition-colors">
              Continue Shopping
            </Link>
          </div>
        </div>
